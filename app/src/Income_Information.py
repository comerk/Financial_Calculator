...
class Pay_Slip:
    def __init__(self, data):
        ...
        gross_amount = data["Gross Amount"]
        self.pay_slip_info["Gross Income"] = round(gross_amount, 2)

        net_amount = data["Net Amount"]
        self.pay_slip_info["Net Income"] = round(net_amount, 2)

        self.pay_slip_info["Taxed Amount"] = round(gross_amount - net_amount, 2)

        self.pay_slip_info["Percent Taxed"] = round(
            self.pay_slip_info["Taxed Amount"] / gross_amount, 3
        )
...
class Quarter:
    ...
    def __str__(self):
        ...
        for stat in self.quarter_income_info:
            string += (
                "Total " + stat + ": " + str(self.quarter_income_info[stat]) + "\t"
            )
    ...
    def Update_Income_Values(self):
        ...
        for pay_slip in self.income_data_for_quarter:
            self.quarter_income_info["Gross Income"] += pay_slip.pay_slip_info[
                "Gross Income"
            ]
            self.quarter_income_info["Net Income"] += pay_slip.pay_slip_info[
                "Net Income"
            ]
            self.quarter_income_info["Taxed Amount"] += pay_slip.pay_slip_info[
                "Taxed Amount"
            ]

        self.quarter_income_info["Percent Taxed"] = round(
            self.quarter_income_info["Taxed Amount"]
            / self.quarter_income_info["Gross Income"],
            2,
        )
...
class Taxable_Year:
    ...
    def __str__(self):
        ...
        for stat in self.year_income_info:
            string += (
                "   " + "Total " + stat + ": " + str(self.year_income_info[stat]) + "\t"
            )
            if stat == "Net Income" or stat == "Percent Taxed":
                string += "\n"
    ...
    def Add_Pay_Slip_To_Quarter(self, pay_slip):
        for quarter in quarter_dates:
            if (
                pay_slip.pay_date.month >= quarter_dates[quarter][0].month
                and pay_slip.pay_date.month <= quarter_dates[quarter][1].month
            ):
                if (
                    pay_slip.pay_date.day >= quarter_dates[quarter][0].day
                    and pay_slip.pay_date.day <= quarter_dates[quarter][1].day
                ):
    ...
    def Update_Income_Values(self):
        ...
        for pay_slip in self.pay_slips:
            self.year_income_info["Gross Income"] += pay_slip.pay_slip_info[
                "Gross Income"
            ]
            self.year_income_info["Net Income"] += pay_slip.pay_slip_info["Net Income"]
            self.year_income_info["Taxed Amount"] += pay_slip.pay_slip_info[
                "Taxed Amount"
            ]

        self.year_income_info["Percent Taxed"] = round(
            self.year_income_info["Taxed Amount"]
            / self.year_income_info["Gross Income"],
            3,
        )
...
def Sort_Data_Into_Years(raw_data):
    ...
    for row in range(1, raw_data.shape[0], 1):
        ...
        if pay_slip.pay_date.year not in taxable_years.keys():
            taxable_years[pay_slip.pay_date.year] = Taxable_Year(pay_slip.pay_date.year)
